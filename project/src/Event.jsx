          ...
          <ul className={styled.event_list}>
            {
              data.map((value, i) => {
                return (
                  data[i].title === "not open" ? 
                  null : 
                  <li key={i} className={styled.item}>
                    <Link to={`/event/${data[i].id}`}>
                      <figure className={styled.figure}>
                        <img src="https://via.placeholder.com/320x320.jpg" alt={data[i].title} />
                      </figure>
                      <div className={styled.text_wrap}>
                        <h3 className={styled.tit}>{data[i].title}</h3>
                        <p className={styled.tit_desc}>{`이벤트 기간: ${data[i].period.slice(0, 4)}년 ${data[i].period.slice(4, 6)}월 ${data[i].period.slice(6, 8)}일 ~ ${data[i].period.slice(9, 13)}월 ${data[i].period.slice(13, 15)}월 ${data[i].period.slice(15, 17)}일`}</p>
                      </div>
                    </Link>
                  </li>
                )
